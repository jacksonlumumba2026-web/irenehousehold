...
function onProductsReady(fn) {
  if (PRODUCTS.length > 0) { fn(); return; }
  _productCallbacks.push(fn);
}

function _runProductCallbacks() {
  var cbs = _productCallbacks.slice(); _productCallbacks = [];
  ...
  fetch('products.json?t=' + Date.now())
    ...
    .then(function(data){
      ...
      if (JSON.stringify(data) !== JSON.stringify(PRODUCTS)) {
        PRODUCTS = data;
        var g = document.getElementById('shop-grid');
        if (g && g.children.length > 0 && typeof applyFilters === 'function') applyFilters();
        var fg = document.getElementById('featured-grid');
        if (fg && fg.children.length > 0 && typeof initHomePage === 'function') initHomePage();
      }
    }).catch(function(){});
}
...
function buildCard(p) {
  ...
  var imgUrl   = 'https://irenehousehold.co.ke/' + p.img;
  ...
  return '<div class="product-card" style="cursor:pointer;" onclick="openQuickView(' + p.id + ')">' +
    ...
    '<div class="card-body">' +
      ...
      '<div style="display:flex;gap:6px;margin-top:8px;">' +
        '<button class="card-atc" data-id="' + p.id + '" data-name="' + n + '" data-cat="' + p.cat + '" data-price="' + p.price + '" data-img="' + p.img + '" style="flex:1;' + (!inStock ? 'background:#ccc;cursor:not-allowed;' : '') + '"' +
...
function handleATC(btn) {
  var p = PRODUCTS.find(function(x){ return x.id === parseInt(btn.dataset.id); });
...
function renderNewArrivals(containerId) {
  var container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = PRODUCTS.slice().reverse().slice(0, 8).map(buildCard).join('');
}
...
function toggleMobileNav(){ var el=document.getElementById('mobile-nav'); if(el) el.classList.toggle('open'); }
function closeMobileNav(){  var el=document.getElementById('mobile-nav'); if(el) el.classList.remove('open'); }
...
function updateDeliveryBar() {
  ...
  fill.style.background = pct>=100 ? '#22c55e' : '';
  if (msg) {
    if (pct >= 100) {
      msg.innerHTML = '&#127881; <strong style="color:#22c55e;">FREE delivery unlocked!</strong>';
    } else {
      msg.innerHTML = 'Add <strong style="color:var(--green);">KSh ' + (FREE_DELIVERY_THRESHOLD-total).toLocaleString() + '</strong> more for <strong style="color:var(--green);">FREE delivery</strong>';
    }
  }
}
...
function addToCart(btn, id, name, cat, price, img) {
  ...
  else { cart.push({id:id, name:name, cat:cat, price:price, img:img, qty:1}); }
  if (btn) {
...
function selectPayStatus(type) {
  payStatus = type;
  var paid=document.getElementById('opt-paid'), nopay=document.getElementById('opt-nopay');
...
function checkout() {
  ...
  var lines = cart.map(function(item){
    var imgUrl    = 'https://irenehousehold.co.ke/' + item.img;
...
function removeFromWishlist(id){
  ...
  updateWishlistUI(); renderWishlistItems();
}
function openWishlist(){
  renderWishlistItems();
  var d=document.getElementById('wl-drawer'),o=document.getElementById('wl-overlay');
  if(d)d.classList.add('open');if(o)o.classList.add('open');
  document.body.style.overflow='hidden';
}
...
function openQuickView(id) {
  ...
  mainImg.style.cssText='width:100%;aspect-ratio:1;object-fit:cover;display:block;cursor:zoom-in;transition:opacity .2s;';
  ...
  if(imgs.length>1){
    ...
    imgs.forEach(function(src,i){
      ...
      th.src=src; th.className='qv-thumb';
  ...
  /* Animate in */
  setTimeout(function(){ modal.style.opacity='1'; sheet.style.transform='translateY(0)'; }, 10);
  document.body.style.overflow='hidden';

  /* Auto slideshow */
  if(imgs.length>1){
    var userPaused=false, slideIdx=0, loadedImgs=[], checked=0, slideTimer=null;
    imgs.forEach(function(src,i){
      var tester=new Image();
      tester.onload=function(){ loadedImgs.push({src:src,idx:i}); loadedImgs.sort(function(a,b){return a.idx-b.idx;}); checked++; if(checked===imgs.length) startSlide(); };
      tester.onerror=function(){ checked++; if(checked===imgs.length) startSlide(); };
      tester.src=src;
    });
    function startSlide(){
      if(loadedImgs.length<2) return;
      slideTimer=setInterval(function(){
        if(userPaused||!document.getElementById('qv-modal')){clearInterval(slideTimer);return;}
        ...
        setTimeout(function(){mImg.src=item.src;mImg.style.opacity='1';},350);
    ...
    if(thumbRow){
      thumbRow.querySelectorAll('img').forEach(function(th){
        var orig=th.onclick;
        th.onclick=function(){userPaused=true;if(orig)orig.call(this);};
      });
    }
  }
}
...
function closeQuickView(){
  ...
  setTimeout(function(){modal.remove();document.body.style.overflow='';},320);
}
...
function openZoom(src){
  ...
  img.src=src; img.style.cssText='max-width:95vw;max-height:90vh;object-fit:contain;border-radius:8px;';
  ...
  ov.appendChild(img); ov.appendChild(cls);
...
function goToProduct(id){ window.location='product.html?id='+id; }
