...
═══════════════════════════════════════ */
/* ═══════════════════════════════════════
   PRODUCTS — Inline for instant load
   Updated via admin panel (products.json)
═══════════════════════════════════════ */
...
/* onProductsReady — runs fn immediately since products are inline */
var _productCallbacks = [];

function onProductsReady(fn) {
  if (fn) fn(); // products already available
}
...
/* loadProducts — fetches fresh data from admin saves (optional refresh) */
function loadProducts(callback) {
  // Products already loaded inline - just run callback
  if (callback) callback();
  // Also try to get latest from products.json in background
  fetch('products.json?v=' + Date.now())
    ...
    .catch(function() {}); // silent fail — inline data still works
}
...
function buildCard(p) {
  ...
  var oos = !inStock
    ? '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.38);"><span style="background:#e53e3e;color:#fff;font-size:9px;font-weight:700;padding:4px 10px;border-radius:3px;">OUT OF STOCK</span></div>'
    : '';
    ...
    '<div class="card-body">' +
      ...
      '<div style="display:flex;gap:6px;margin-top:8px;">' +
        '<button class="card-atc" data-id="' + p.id + '" data-name="' + n + '" data-cat="' + p.cat + '" data-price="' + p.price + '" data-img="' + p.img + '" style="flex:1;' + (!inStock ? 'background:#ccc;cursor:not-allowed;' : '') + '" ' +
          (inStock ? 'onclick="event.stopPropagation();handleCardATC(this)"' : 'disabled') +
...
function copyValue(elemId, btn) {
  var val = document.getElementById(elemId).textContent;
