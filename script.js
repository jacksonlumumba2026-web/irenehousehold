...
function openQuickView(id) {
  ...
  if (!modal) {
    ...
    modal.id = 'qv-modal';
    modal.style.cssText = 'position:fixed;inset:0;z-index:3000;background:rgba(0,0,0,.55);display:none;align-items:flex-end;justify-content:center;opacity:0;transition:opacity .25s;';
  ...
  setTimeout(function(){ box.style.transition = 'transform .35s cubic-bezier(.25,.46,.45,.94)'; }, 0);

  box.innerHTML = '';

  /* ── Drag handle ── */
  var handle = document.createElement('div');
  handle.style.cssText = 'width:36px;height:4px;background:#e0e0e0;border-radius:2px;margin:10px auto 0;';
  ...
  closeBtn.style.cssText = 'position:absolute;top:12px;right:12px;z-index:10;background:rgba(0,0,0,.08);border:none;color:#333;width:30px;height:30px;border-radius:50%;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;';
  box.appendChild(closeBtn);

  /* ── Main image ── */
  var imgWrap = document.createElement('div');
  imgWrap.style.cssText = 'position:relative;width:100%;background:#f5f5f5;overflow:hidden;margin-top:6px;';

  var mainImg = document.createElement('img');
  mainImg.id  = 'qv-main-img';
  mainImg.src = imgs[0];
  mainImg.alt = p.name;
  mainImg.style.cssText = 'width:100%;aspect-ratio:1;object-fit:cover;display:block;transition:opacity .2s;cursor:zoom-in;';
  mainImg.onclick  = function(){ openZoom(this.src); };
  mainImg.onerror  = function(){
    this.style.display = 'none';
    imgWrap.style.background = 'linear-gradient(135deg,#e8f5ee,#d4eadd)';
    imgWrap.style.aspectRatio = '1';
    imgWrap.style.display = 'flex';
    imgWrap.style.alignItems = 'center';
    imgWrap.style.justifyContent = 'center';
    imgWrap.innerHTML += '<span style="font-size:3.5rem;opacity:.2;">&#128247;</span>';
  };
  imgWrap.appendChild(mainImg);

  /* Badge */
  if (p.badge) {
    var bdgColors = { HOT:'background:#c8a951;color:#0d1f16', SALE:'background:#e53e3e;color:#fff', NEW:'background:#1a3d2b;color:#fff', HOTEL:'background:#7c3aed;color:#fff' };
    var bdg = document.createElement('span');
    bdg.textContent = p.badge;
    bdg.style.cssText = 'position:absolute;top:12px;left:12px;' + (bdgColors[p.badge]||'background:#333;color:#fff') + ';font-size:10px;font-weight:700;padding:4px 10px;border-radius:4px;letter-spacing:.5px;';
    imgWrap.appendChild(bdg);
  }

  /* Zoom hint */
  var zh = document.createElement('div');
  zh.innerHTML = '&#128269; Tap to zoom';
  zh.style.cssText = 'position:absolute;bottom:10px;right:10px;background:rgba(0,0,0,.4);color:#fff;font-size:9px;padding:3px 8px;border-radius:4px;pointer-events:none;';
  imgWrap.appendChild(zh);
  ...
  if (imgs.length > 1) {
    ...
    strip.style.cssText = 'display:flex;gap:8px;padding:10px 14px;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;background:#fff;border-bottom:1px solid #f0f0f0;';
    ...
    imgs.forEach(function(src, i) {
      ...
      th.style.cssText = [
        ...
        'border-radius:8px',
        'cursor:pointer',
        'border:2.5px solid ' + (i === 0 ? '#1a3d2b' : '#e8e8e8'),
        'transition:all .18s'
      ].join(';');

      th.onclick = (function(src, el) {
        return function() {
          ...
          strip.querySelectorAll('img').forEach(function(t){
            t.style.border = '2.5px solid #e8e8e8';
  ...
  info.style.cssText = 'padding:16px 16px 28px;';
  ...
  title.style.cssText = 'font-family:Cormorant Garamond,serif;font-size:1.45rem;color:#0d1f16;margin-bottom:5px;line-height:1.25;';
  info.appendChild(title);

  var desc = document.createElement('p');
  desc.textContent = p.note;
  desc.style.cssText = 'font-size:12.5px;color:#6b7280;line-height:1.6;margin-bottom:14px;';
  info.appendChild(desc);

  /* Price block */
  var priceBlock = document.createElement('div');
  priceBlock.style.cssText = 'display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:14px;padding:12px 14px;background:#f8f8f8;border-radius:10px;border-left:4px solid #c8a951;';

  var priceEl = document.createElement('span');
  priceEl.textContent = price;
  priceEl.style.cssText = 'font-family:Cormorant Garamond,serif;font-size:1.9rem;font-weight:700;color:#1a3d2b;line-height:1;';
  priceBlock.appendChild(priceEl);

  if (p.old) {
    var oldEl = document.createElement('span');
    oldEl.textContent = 'KSh ' + p.old.toLocaleString();
    oldEl.style.cssText = 'font-size:13px;color:#bbb;text-decoration:line-through;';
    priceBlock.appendChild(oldEl);

    if (p.price > 0) {
      var saveEl = document.createElement('span');
      saveEl.textContent = 'Save KSh ' + (p.old - p.price).toLocaleString();
      saveEl.style.cssText = 'font-size:10px;background:#fff0f0;color:#e53e3e;font-weight:700;padding:2px 8px;border-radius:4px;';
      priceBlock.appendChild(saveEl);
    }
  }
  ...
  waBtn.style.cssText = 'display:block;width:100%;background:#25d366;color:#fff;padding:14px;font-size:13.5px;font-weight:700;border-radius:11px;text-align:center;text-decoration:none;margin-bottom:9px;';
  ...
  cartBtn.style.cssText = 'width:100%;background:#1a3d2b;color:#fff;border:none;padding:13px;font-size:13px;font-weight:700;border-radius:11px;cursor:pointer;margin-bottom:9px;';
  ...
  wlBtn.style.cssText = 'width:100%;background:#fff;color:#1a3d2b;border:1.5px solid #e0e0e0;padding:12px;font-size:12px;font-weight:600;border-radius:11px;cursor:pointer;margin-bottom:16px;';
  info.appendChild(wlBtn);

  /* Trust row */
  var trust = document.createElement('div');
  trust.style.cssText = 'background:#f8f8f8;border-radius:10px;padding:12px 14px;display:flex;flex-direction:column;gap:7px;margin-bottom:14px;';
  [
    ...
    row.innerHTML = t;
    row.style.cssText = 'font-size:11.5px;color:#6b7280;display:flex;align-items:center;gap:7px;';
  ...
  fpLink.style.cssText = 'display:block;text-align:center;font-size:12px;color:#1a3d2b;font-weight:600;border:1.5px solid #d0d0d0;padding:11px;border-radius:11px;text-decoration:none;';
